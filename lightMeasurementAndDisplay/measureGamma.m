...
meter = 'colorCal'; %'colorCal' or 'UDT'
steps = 256; %number of steps recorded from. 
...
for channel = 1:length(name)
    ...
    for level = 0:steps
        ...
        drawColorToDisplay(colorCode(channel,:).*(level/(steps)),display,window,windowDimensions)
    ...
    scatter(gamma(:,1),gamma(:,2),20,'filled','MarkerFaceColor',colorCode(channel,:),'MarkerEdgeColor',[0 0 0]);hold on
    xlabel('Display Setting')
    ylabel('Luminance')
    title('Gamma Function Measurement')
    legend(sprintf('val(x) = %f*x^{%f}+%f',gammaCoefficients(1,channel),gammaCoefficients(2,channel),gammaCoefficients(3,channel)))
end
